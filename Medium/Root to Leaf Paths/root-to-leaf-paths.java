...
class Solution {
    ...
    public static void helperDfs(Node root, ArrayList<Integer> list,ArrayList<ArrayList<Integer>> ans){
        if(root == null)return;
        list.add(root.data);
        if(root.left == null && root.right == null)ans.add(new ArrayList<>(list));
        helperDfs(root.left, list, ans);
        helperDfs(root.right, list, ans);
